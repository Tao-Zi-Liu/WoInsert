...
export default function DataEntryPage() {
  ...
  const columnWidths = {
    woid: isCollapsed ? "w-[180px]" : "w-[150px]",
    wlid: isCollapsed ? "w-[180px]" : "w-[150px]",
    quantity: isCollapsed ? "w-[140px]" : "w-[120px]",
    startDate: isCollapsed ? "w-[200px]" : "w-[180px]",
    endDate: isCollapsed ? "w-[200px]" : "w-[180px]",
    department: isCollapsed ? "w-[200px]" : "w-[180px]",
    action: "w-[50px]"
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className={columnWidths.woid}>
                    {t.form.workOrderId} <span className="text-red-500">*</span>
                  </TableHead>
                  <TableHead className={columnWidths.wlid}>
                    {t.form.workListId} <span className="text-red-500">*</span>
                  </TableHead>
                  <TableHead className={columnWidths.quantity}>
                    {t.form.quantity} <span className="text-red-500">*</span>
                  </TableHead>
                  <TableHead className={columnWidths.startDate}>
                    {t.form.startDate} <span className="text-red-500">*</span>
                  </TableHead>
                  <TableHead className={columnWidths.endDate}>
                    {t.form.endDate} <span className="text-red-500">*</span>
                  </TableHead>
                  <TableHead className={columnWidths.department}>
                    {t.form.department} <span className="text-red-500">*</span>
                  </TableHead>
                  <TableHead className={cn(columnWidths.action, "text-right")}>{t.form.action}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {fields.length > 0 ? (
                                    ...
                                    <Popover>
                                      <PopoverTrigger asChild>
                                        ...
                                        >
                                          ...
                                          {controllerField.value ? format(new Date(controllerField.value), "MMM dd, yyyy") : <span>{t.form.pickDate}</span>}
                                        </Button>
                                      </PopoverTrigger>
